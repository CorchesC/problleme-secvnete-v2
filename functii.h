...
void secventiereSol2(int v[], int dim) {
	...
	int dmax = 0;
	for (int i = 0; i < dim; i++) {
		if (numarPrim(v[i]) == true && numarPrim(v[i+1]) == true  && v[i]< v[i+1] &&  numarPrim(oglindit(v[i])) == true && numarPrim(oglindit(v[i])) == true) {
			int j = i;
			while (j + 1 < dim && numarPrim(v[j]) == true && numarPrim(v[j+ 1]) == true && v[j] < v[j+ 1] && numarPrim(oglindit(v[j])) == true && numarPrim(oglindit(v[j])) == true) {
				j++;
			}
